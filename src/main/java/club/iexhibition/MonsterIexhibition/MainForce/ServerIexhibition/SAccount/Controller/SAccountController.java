package club.iexhibition.MonsterIexhibition.MainForce.ServerIexhibition.SAccount.Controller;
...
public class SAccountController {
     ...
     * @return
     */
    @ApiOperation(value = "账户登录",httpMethod = "POST")
    @PostMapping(value = "/login")
    public ResponseResult login(@RequestBody Login login){
        try {
            LoginAccount la = accountService.login(login.getUsername(), login.getPassword());
            if(la!=null){
                return new ResponseResult(CommonCode.SUCCESS,la);
            }else {
                return new ResponseResult(CommonCode.PASSWORD_ERROR);
            }
        }catch (Exception e){
            log.error("账户登录异常");
            log.error(e);
